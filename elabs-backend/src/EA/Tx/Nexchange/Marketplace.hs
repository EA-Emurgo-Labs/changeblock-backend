...
deployScript toAddr marketplaceParams scripts =
  let mktValidator = marketplaceValidator marketplaceParams scripts
      out = GYTxOut toAddr (valueFromLovelace 100) Nothing (Just $ GYPlutusScript $ validatorToScript mktValidator)
  ...
  where
    ...
    prepareTx mktInfo@MarketplaceInfo {..} usedAmt =
      let changeAmt = mktInfoAmount - usedAmt
       ...
       in if changeAmt > 0
            then inTx <> mustHaveOutput (mkGYTxOut mktInfoAddress (mkCarbontokenValue mktInfo changeAmt) (datumFromPlutusData newDatum))
            else inTx